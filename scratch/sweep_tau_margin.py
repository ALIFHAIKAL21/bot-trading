...
# Sweep Tau and Margin on 2026, 2025, 2024, 2023
...
def run_filtered(tau_val, margin_val, year):
            ...
            for b_idx in day_idxs:
                ...
                if w_st <= t_val <= w_en:
                    ...
                    if 0 <= p_idx < len(preds):
                        ...
                        if conf > best_conf and conf >= tau_val and margin >= margin_val:
                            best_conf = conf
                            best_idx = b_idx
                            best_act = max_i + 1
    ...
    return {'pnl': net, 'wr': wr, 'dd': max_dd, 'dip': min_eq, 'trades': n_trades}

print("SWEEPING TAU / MARGIN ON 2026:")
for tau in [0.32, 0.35, 0.38, 0.40]:
    for marg in [0.01, 0.02, 0.03, 0.05]:
        r26 = run_filtered(tau, marg, 2026)
        print(f"Tau: {tau:.2f} | Marg: {marg:.2f} -> PnL: ${r26['pnl']:+7.2f} | WR: {r26['wr']:.1f}% | DD: {r26['dd']:.2f}% | Dip: ${r26['dip']:.2f} | Tr: {r26['trades']}")
