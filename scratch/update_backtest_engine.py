"""Update src/validation/backtest_engine.py with Twin-Order Partial TP and Trailing Stop"""
import pathlib
...
(StructuralRiskEngine) dengan biaya friksi realistis, Twin-Order Partial Take Profit,
dan Trailing Stop dinamis.

Fitur:
- Bar-by-bar sequential processing (zero lookahead bias)
- Full risk gate integration: News Blackout, Friday Rule, Circuit Breaker,
  Concurrency, Confidence Threshold, Max Daily Entries Cap
- Twin-Order Partial Take Profit: 50% lot @ TP1 (+1.0R), 50% lot @ TP2 (+2.0R)
- Widened Trailing Breakeven (+1.0R trigger) & Dynamic Trailing Stop
- Realistic friction: spread + commission + adverse slippage (2-way)
- Comprehensive metrics: PnL, Profit Factor, Sharpe, Sortino, Max DD, Win Rate
"""
...
class RealisticBacktestEngine:
    """
    Mesin backtest event-driven realistis untuk XAU/USD M30.
    """
    ...
    def _calculate_lot_size(
        ...
        lot = max(0.02, min(lot, 50.0))  # At least 0.02 so half is >= 0.01
    ...
    def run(
        ...
        for bar_idx in range(n_bars):
            ...
            if open_position is not None:
                ...
                lot_half = open_position["lot_half"]
                ...
                if is_friday_close or is_time_barrier:
                    ...
                    if open_position["pos_a_open"]:
                        pnl_a_g = (exit_price - ep) * lot_half * self.contract_size if d == "BUY" else (ep - exit_price) * lot_half * self.contract_size
                        f_a = self._friction_per_trade(lot_half)
                        open_position["pnl_net_accum"] += (pnl_a_g - f_a)
                        open_position["pos_a_open"] = False

                    if open_position["pos_b_open"]:
                        pnl_b_g = (exit_price - ep) * lot_half * self.contract_size if d == "BUY" else (ep - exit_price) * lot_half * self.contract_size
                        f_b = self._friction_per_trade(lot_half)
                    ...
                    if d == "BUY":
                        ...
                        if not open_position["tp1_hit"] and bar_low <= open_position["cur_sl_a"]:
                            ...
                            if open_position["pos_a_open"] and bar_high >= open_position["tp1_price"]:
                                pnl_a_g = (open_position["tp1_price"] - ep) * lot_half * self.contract_size
                                f_a = self._friction_per_trade(lot_half)
                                open_position["pnl_net_accum"] += (pnl_a_g - f_a)
                                open_position["pos_a_open"] = False
                            ...
                            if open_position["pos_b_open"]:
                                if bar_high >= open_position["tp2_price"]:
                                    pnl_b_g = (open_position["tp2_price"] - ep) * lot_half * self.contract_size
                                    f_b = self._friction_per_trade(lot_half)
                                    open_position["pnl_net_accum"] += (pnl_b_g - f_b)
                                    open_position["pos_b_open"] = False
                                    ...
                                    pnl_b_g = (open_position["cur_sl_b"] - ep) * lot_half * self.contract_size
                                    f_b = self._friction_per_trade(lot_half)
                                    open_position["pnl_net_accum"] += (pnl_b_g - f_b)
                                    open_position["pos_b_open"] = False
                        ...
                        if not open_position["tp1_hit"] and bar_high >= open_position["cur_sl_a"]:
                            ...
                            if open_position["pos_a_open"] and bar_low <= open_position["tp1_price"]:
                                pnl_a_g = (ep - open_position["tp1_price"]) * lot_half * self.contract_size
                                f_a = self._friction_per_trade(lot_half)
                                open_position["pnl_net_accum"] += (pnl_a_g - f_a)
                                open_position["pos_a_open"] = False
                                open_position["tp1_hit"] = True
                            ...
                            if open_position["pos_b_open"]:
                                if bar_low <= open_position["tp2_price"]:
                                    pnl_b_g = (ep - open_position["tp2_price"]) * lot_half * self.contract_size
                                    f_b = self._friction_per_trade(lot_half)
                                    open_position["pnl_net_accum"] += (pnl_b_g - f_b)
                                    open_position["pos_b_open"] = False
                                    open_position["tp2_hit"] = True
                                    ...
                                    exit_r = "BE_HIT" if open_position["tp1_hit"] else "SL_HIT"
                                    pnl_b_g = (ep - open_position["cur_sl_b"]) * lot_half * self.contract_size
                                    f_b = self._friction_per_trade(lot_half)
                                    open_position["pnl_net_accum"] += (pnl_b_g - f_b)
                                    open_position["pos_b_open"] = False
