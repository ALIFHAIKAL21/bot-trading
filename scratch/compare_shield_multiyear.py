...
# Compare 2025, 2024, 2023 with and without 2-Loss Daily Shield
...
def evaluate_year(year, max_daily_losses=None):
    st_dt = np.datetime64(f'{year}-01-01T00:00:00')
    en_dt = np.datetime64(f'{year}-12-31T23:59:59')
...
print("="*80)
print("MULTI-YEAR AUDIT: BASELINE vs 2-LOSS DAILY SHIELD")
print("="*80)
for yr in [2025, 2024, 2023]:
    b = evaluate_year(yr, max_daily_losses=None)
    s = evaluate_year(yr, max_daily_losses=2)
    print(f"[{yr}] BASELINE : PnL: ${b['net_pnl']:+8.2f} | WR: {b['win_rate']:.1f}% | Max DD: {b['max_dd']:.2f}% | Trades: {b['trades']}")
    print(f"[{yr}] SHIELD   : PnL: ${s['net_pnl']:+8.2f} | WR: {s['win_rate']:.1f}% | Max DD: {s['max_dd']:.2f}% | Trades: {s['trades']}")
    diff_pnl = s['net_pnl'] - b['net_pnl']
    print(f"       DELTA  : PnL Diff: ${diff_pnl:+7.2f} | Trade Diff: {s['trades'] - b['trades']}")
    print("-"*80)
