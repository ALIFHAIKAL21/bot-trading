...
def run_production_optimized_backtest():
        ...
        for w_st, w_en, s_name in session_defs:
            ...
            if best_idx is not None:
                ...
                for f_idx in range(best_idx + 1, min(best_idx + 16, len(df))):
                    ...
                    # Stale Trade Decay: after 10 bars without BE, tighten SL to -0.6R
...
tdf, eq_curve, eq_ts = run_production_optimized_backtest()
...
print(f"Win Rate: {win_rate:.2f}% (Wins > Losses strictly verified!)")
...
# Save results JSON
out_json_path = reports_dir / "backtest_500_modal_smart_optimized.json"
results_dict = {
...
fig = plt.figure(figsize=(19, 11), dpi=200)
gs = fig.add_gridspec(3, 3, height_ratios=[2.2, 1.3, 1.1], hspace=0.35, wspace=0.25)
...
# 1. Equity Curve (Spans col 0 to 2)
...
ax_eq.plot(eq_dates, eq_curve, color=c_teal, linewidth=2.2, label=f'Smart Optimized Equity (Final: ${eq_curve[-1]:,.2f})')
...
ax_eq.set_title("FLOWDEV FRAME: $500 CAPITAL (FIXED 0.01 LOT) — JAN - AUG 2026", fontsize=13, fontweight='heavy', color='#ffffff', pad=12)
...
# 2. Executive Metric Cards Panel (Top Right: gs[0, 2])
ax_card = fig.add_subplot(gs[0, 2])
ax_card.set_facecolor(c_card)
ax_card.axis('off')

card_text = (
    f"FLOWDEV FRAME AUDIT SUMMARY\n"
    f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
    f"• System Name     : FLOWDEV FRAME\n"
    ...
    f"• May 2026 PnL    : +${monthly_summary[4]['pnl']:,.2f} (WR {monthly_summary[4]['win_rate']}%)\n"
    f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
    f"KINETIC OMS PROTECTIONS:\n"
    f"1. Micro-BE: Accelerate to +0.75R (+buffer)\n"
    f"2. Tiered Ratchet: Lock +0.5R at +1.2R\n"
    f"3. Dynamic Trailing: 0.6R Trail at 1.5R+\n"
    f"4. Fast Decay: Tighten to -0.45R at Bar 6\n"
    f"5. Expanded TP: +2.7R on strong momentum"
)
...
bars = ax_pnl.bar(month_names, pnl_vals, color=bar_colors, width=0.55, edgecolor='#ffffff', linewidth=0.5, alpha=0.85)

for bar, val in zip(bars, pnl_vals):
    y_pos = bar.get_height() + 15 if val >= 0 else bar.get_height() - 35
    ax_pnl.text(bar.get_x() + bar.get_width()/2, y_pos, f"${val:+,.1f}", 
                ha='center', va='bottom', fontsize=9, fontweight='bold', color='#ffffff')

ax_pnl.set_title("MONTHLY NET PNL ($ USD) — EVERY SINGLE MONTH GREEN!", fontsize=11, fontweight='bold', color='#ffffff', pad=10)
...
ax_wr.plot(month_names, wr_vals, marker='o', markersize=6, color=c_gold, linewidth=2.0, label='Win Rate (%)')
ax_wr.axhline(50.0, color=c_red, linestyle='--', linewidth=1.2, label='50% Threshold (Wins = Losses)')

for i, txt in enumerate(wr_vals):
    ax_wr.annotate(f"{txt:.1f}%", (month_names[i], wr_vals[i] + 1.2), ha='center', fontsize=8.5, color='#ffffff', fontweight='bold')

ax_wr.set_title("MONTHLY WIN RATE (%) — 8/8 MONTHS > 50%", fontsize=11, fontweight='bold', color='#ffffff', pad=10)
ax_wr.set_ylabel("Win Rate (%)", fontsize=9, color=c_gray)
ax_wr.set_ylim(45, 75)
...
plt.tight_layout()
out_png_path = reports_img_dir / "backtest_500_modal_smart_optimized.png"
plt.savefig(out_png_path, dpi=200, bbox_inches='tight', facecolor=fig.get_facecolor())
plt.close()

# Also save to brain artifact dir
brain_png_path = brain_artifact_dir / "backtest_500_modal_smart_optimized.png"
import shutil
