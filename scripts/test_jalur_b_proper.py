...
# H4 Data strictly lagged (zero lookahead)
...
df_h4_lagged = df_h4[['h4_ema20', 'h4_ema50', 'h4_ema200', 'h4_slope20', 'h4_slope50', 'h4_high_20d', 'h4_low_20d', 'h4_ath_proximity']].shift(1)
df = df.merge(df_h4_lagged, on='timestamp_utc', how='left')
for col in ['h4_ema20', 'h4_ema50', 'h4_ema200', 'h4_slope20', 'h4_slope50', 'h4_high_20d', 'h4_low_20d', 'h4_ath_proximity']:
...
def run_simulation(strategy_name, risk_scaling=False, uncapped_trend=False, scalp_counter=False):
    seq_offset = 63
    n_bars = len(df)
    results = {}
    ...
    for y_cfg in test_years:
        ...
        for bar_idx in range(n_bars):
            ...
            if open_position is not None:
                ...
                is_scalp = open_position.get("is_scalp", False)
                    ...
                    if d == "BUY":
                        if not open_position["tp1_hit"] and b_low <= open_position["cur_sl_a"]:
                            ...
                            if open_position["pos_b_open"]:
                                # In scalp mode, TP2 is hit at 1.2R (fast profit taking)
                                target_tp2 = open_position["tp2_price"]
                                if is_scalp:
                                    target_tp2 = round(ep + (1.2 * sl_dist), 2)
                                    
                                if not is_runner and b_high >= target_tp2:
                        ...
                        if not open_position["tp1_hit"] and b_high >= open_position["cur_sl_a"]:
                            ...
                            if open_position["pos_b_open"]:
                                ...
                                if not is_runner and b_low <= target_tp2:
                ...
                if not fri_freeze and not lon_quar:
                    ...
                    if conf >= TAU and conf > p_hold:
                        ...
                        if ok:
                            ...
                            is_parabolic_bull = (s20 > 0.20) and ath
                            is_parabolic_bear = (s20 < -0.20)
                            
                            # Macro alignment check
                            macro_aligned = (d == "BUY" and s50 > 0.1 and c > e50) or (d == "SELL" and s50 < -0.1 and c < e50)
                            
                            # Determine risk multiplier and trade style:
                            risk_mult = 1.0
                            is_scalp = False
                            is_runner = False
                            
                            if strategy_name == "baseline":
                                pass # Keep default 1.0x
                            elif strategy_name == "asymmetric_risk":
                                # Never skip a trade!
                                # If aligned with macro trend: 1.0x risk + uncapped runner
                                # If counter to parabolic ATH freight train: 0.20x risk (micro scalp)
                                if is_parabolic_bull and d == "SELL":
                                    risk_mult = 0.20 # Minimal risk exposure
                                    is_scalp = True
                                elif is_parabolic_bear and d == "BUY":
                                    risk_mult = 0.20
                                    is_scalp = True
                                elif macro_aligned and uncapped_trend:
                                    risk_mult = 1.0
                                    is_runner = True
                            elif strategy_name == "smart_asymmetric_runner":
                                # Even better: When aligned with macro trend, let position run!
                                # When counter-trend, keep standard or slight scalp
                                if is_parabolic_bull and d == "SELL":
                                    risk_mult = 0.25
                                    is_scalp = True
                                elif is_parabolic_bear and d == "BUY":
                                    ...
                                    risk_mult = 1.20 # Slight boost when riding macro wave!
                                    is_runner = True
                            elif strategy_name == "h4_reversal_confirmation":
                                # If at parabolic ATH, require M30 candle to be red before entering SELL (price confirmation)
                                if is_parabolic_bull and d == "SELL":
                                    if b_close > opens[bar_idx]: # If candle is still expanding green, don't stand in front of train
                                        ok = False
                            ...
                            if ok:
                                ...
                                if sl_dist > 0:
                                    target_risk = equity * BASE_RISK * risk_mult
                                    ...
                                    open_position = {
                                        ...
                                        "pnl_net_accum": 0.0,
                                        "is_scalp": is_scalp, "is_runner": is_runner
                                    }
                                    daily_count += 1
...
print("\n=======================================================")
print("JALUR B DENGAN BENAR (AUDIT 5 TAHUN: 2021 - 2025)")
print("=======================================================")

print("\n1. BASELINE (STANDAR LAMA):")
base = run_simulation("baseline")
for y, v in base.items():
    print(f"  {y}: Return {v['ret']:+6.2f}% | Max DD {v['max_dd']:6.2f}% | WR {v['wr']:4.1f}% | Trades {v['trades']}")

print("\n2. JALUR B - ASYMMETRIC RISK (NO TRADE SKIPPING! Micro Risk 0.2x on Parabolic Extremes):")
asym = run_simulation("asymmetric_risk", risk_scaling=True, uncapped_trend=False)
for y, v in asym.items():
    print(f"  {y}: Return {v['ret']:+6.2f}% | Max DD {v['max_dd']:6.2f}% | WR {v['wr']:4.1f}% | Trades {v['trades']}")

print("\n3. JALUR B - SMART RUNNER (Trend Alignment Boost 1.2x + Parabolic Scalp 0.25x):")
runner = run_simulation("smart_asymmetric_runner", risk_scaling=True, uncapped_trend=True)
for y, v in runner.items():
    print(f"  {y}: Return {v['ret']:+6.2f}% | Max DD {v['max_dd']:6.2f}% | WR {v['wr']:4.1f}% | Trades {v['trades']}")

print("\n4. JALUR B - PRICE CONFIRMATION (Reject Green Candles into Parabolic Train):")
pconf = run_simulation("h4_reversal_confirmation")
for y, v in pconf.items():
    print(f"  {y}: Return {v['ret']:+6.2f}% | Max DD {v['max_dd']:6.2f}% | WR {v['wr']:4.1f}% | Trades {v['trades']}")
