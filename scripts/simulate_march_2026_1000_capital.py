"""
Simulasi 1 Minggu Maret 2026 dengan Modal $1,000
Memeriksa lot size minimum (0.01 vs 0.02) dan hasil riil untuk trader retail.
"""
...
BASE_RISK = 0.01         # 1% per trade = $10.00
...
for day_str in days:
    ...
    for s_start, s_end, s_name in session_ranges:
        ...
        if best_bar_idx is not None:
            ...
            target_risk = equity * BASE_RISK  # ~$10
            raw_lot = target_risk / (sl_dist * CONTRACT_SIZE)

            # Pada modal $1,000, lot minimum adalah 0.02 lot (0.01 Pos A + 0.01 Pos B)
            # Jika menggunakan micro lot 0.01 lot tunggal:
            lot_a = 0.01
            lot_b = 0.01
            total_lot = 0.02
...
print(f"=== HASIL SIMULASI MODAL $1,000 (2 - 6 MARET 2026) ===")
print(f"Modal Awal : ${INITIAL_EQUITY:,.2f}")
print(f"Saldo Akhir: ${equity:,.2f}")
print(f"Net PnL    : ${equity - INITIAL_EQUITY:+,.2f} ({(equity - INITIAL_EQUITY)/INITIAL_EQUITY*100:+.2f}%)")
print(f"Total Trade: {len(tdf)}")
wins = tdf['win'].sum()
losses = len(tdf) - wins
print(f"Menang / Kalah: {wins} / {losses} (Akurasi: {wins/len(tdf)*100:.1f}%)")
...
for d_str, g in tdf.groupby('day'):
    w_cnt = g['win'].sum()
    print(f"  {d_str}: {len(g)} Trade | Win: {w_cnt} | Loss: {len(g)-w_cnt} | PnL: ${g['pnl'].sum():+6.2f} ({(g['pnl'].sum()/1000)*100:+.2f}%)")

print("\nTABEL RINCIAN 15 TRADE:")
for _, r in tdf.iterrows():
    print(f"  Trade #{r['id']:2d} | {r['day']} | {r['session']} | {r['direction']:4s} | Lot: {r['lot_size']} | PnL: ${r['pnl']:+6.2f} | {r['exit_reason']}")
