"""
Fast Cadence & Calibration Evaluation for Jan - Aug 2026 ($1,000 Capital, 4-5 Trades/Day)
Using Pre-indexed NumPy Arrays for Ultra-Fast Execution (< 1s)
"""
import sys, pathlib
import numpy as np
import pandas as pd
...
def simulate_fast(mode="sequential", max_daily=4, tau_val=0.38, max_conc=2, sizing="fixed_002"):
    ...
    for bar_idx in range(len(df)):
        ...
        if pred_idx >= 0 and pred_idx < len(preds) and len(open_positions) < max_conc and daily_count < max_daily:
    ...
    tr_per_act = n_trades / n_days if n_days > 0 else 0
    
    monthly = tdf.groupby('month')['pnl'].sum().to_dict()
...
print("Running fast evaluation across candidate configurations...")
print(f"{'Tau':>5} | {'Dly':>3} | {'Cnc':>3} | {'Sizing':>11} | {'Trades':>6} | {'Tr/Day':>6} | {'WR%':>5} | {'PF':>5} | {'Net PnL':>10} | {'Ret%':>7} | {'MaxDD%':>7}")
print("-" * 88)

for tau in [0.38, 0.40, 0.41, 0.42]:
    for dly in [4, 5]:
        for cnc in [2, 3]:
            for sz in ["fixed_002", "dynamic_1.5"]:
                r = simulate_fast(max_daily=dly, tau_val=tau, max_conc=cnc, sizing=sz)
                if r:
                    print(f"{r['tau']:5.2f} | {r['max_daily']:3d} | {r['conc']:3d} | {r['sizing']:>11} | {r['trades']:6d} | {r['tr_per_day']:6.2f} | {r['win_rate']:5.1f} | {r['pf']:5.2f} | ${r['net_pnl']:9.2f} | {r['ret_pct']:6.1f}% | {r['max_dd']:6.2f}%")
