...
# TARGET: 2021-09-28 to 2022-12-31
st_dt = np.datetime64('2021-09-28T00:00:00')
...
def run_2021_2022_backtest():
    equity = INITIAL_EQUITY
    equity_curve = [equity]
    equity_timestamps = [ts_arr[indices[0]]]
    trades = []
...
print("Executing Locked Production Backtest for 2021 - 2022...")
tdf, eq_curve, eq_ts = run_2021_2022_backtest()
...
# Year Breakdown
year_summary = []
for y_str in sorted(tdf['year'].unique()):
    y_grp = tdf[tdf['year'] == y_str]
    y_tr = len(y_grp)
    y_w = y_grp['win'].sum()
    y_wr = y_w / y_tr * 100
    y_pnl = y_grp['pnl'].sum()
    year_summary.append({
        ...
        'wins': int(y_w),
        'losses': int(y_tr - y_w),
        'win_rate': round(y_wr, 2),
        'pnl': round(y_pnl, 2)
    })

print(f"\n=== FULL AUDIT RESULTS: 2021 - 2022 ===")
...
print("\n--- Year Summary ---")
print(pd.DataFrame(year_summary).to_string(index=False))
...
# Save results JSON
out_json_path = reports_dir / "backtest_500_modal_2021_2022.json"
results_dict = {
...
# ================= PLOTTING HIGH-END TEARSHEET FOR 2021-2022 =================
...
ax_eq.plot(eq_dates, eq_curve, color=c_teal, linewidth=2.2, label=f'2021-2022 Locked Equity (Final: ${eq_curve[-1]:,.2f})')
...
ax_eq.set_title("EQUITY GROWTH CURVE: $500 CAPITAL (FIXED 0.01 LOT) — 2021 TO 2022", fontsize=13, fontweight='heavy', color='#ffffff', pad=12)
...
card_text = (
    ...
    f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
    f"YEAR SUMMARY:\n"
    f"• 2021 (Q4) : +${year_summary[0]['pnl']:,.2f} (WR {year_summary[0]['win_rate']}%)\n"
    f"• 2022 (Full): +${year_summary[1]['pnl']:,.2f} (WR {year_summary[1]['win_rate']}%)\n"
    f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
...
ax_card.text(0.06, 0.5, card_text, fontsize=9.0, fontfamily='monospace', color='#e0e0e0',
...
month_labels = [m['month'] for m in monthly_summary]
bar_colors = [c_teal if p >= 0 else c_red for p in pnl_vals]
bars = ax_pnl.bar(month_labels, pnl_vals, color=bar_colors, width=0.6, edgecolor='#ffffff', linewidth=0.5, alpha=0.85)

for bar, val in zip(bars, pnl_vals):
    y_pos = bar.get_height() + 10 if val >= 0 else bar.get_height() - 25
    ax_pnl.text(bar.get_x() + bar.get_width()/2, y_pos, f"${val:+,.0f}", 
                ha='center', va='bottom', fontsize=7.5, fontweight='bold', color='#ffffff', rotation=45)

ax_pnl.set_ylim(0, max(pnl_vals) * 1.25)
ax_pnl.set_title("2021 - 2022 MONTHLY NET PNL ($ USD)", fontsize=11, fontweight='bold', color='#ffffff', pad=14)
...
ax_wr.plot(month_labels, wr_vals, marker='o', markersize=4, color=c_gold, linewidth=1.5, label='Win Rate (%)')
ax_wr.axhline(50.0, color=c_red, linestyle='--', linewidth=1.2, label='50% Threshold')
ax_wr.set_title("MONTHLY WIN RATE (%) — 2021-2022", fontsize=11, fontweight='bold', color='#ffffff', pad=10)
...
out_png_path = reports_img_dir / "backtest_500_modal_2021_2022.png"
plt.savefig(out_png_path, dpi=200, bbox_inches='tight', facecolor=fig.get_facecolor())
plt.close()

# Also save to brain artifact dir
brain_png_path = brain_artifact_dir / "backtest_500_modal_2021_2022.png"
import shutil
shutil.copyfile(out_png_path, brain_png_path)

print(f"\nSaved 2021-2022 chart tearsheet to: {out_png_path}")
