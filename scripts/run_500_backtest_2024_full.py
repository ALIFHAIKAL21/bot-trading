"""
Production Institutional Backtest: FULL YEAR 2024 (All 12 Months)
...
# TARGET: FULL YEAR 2024 (Jan 1, 2024 to Dec 31, 2024)
st_dt = np.datetime64('2024-01-01T00:00:00')
en_dt = np.datetime64('2024-12-31T23:59:59')
mask_2024 = (ts_arr >= st_dt) & (ts_arr <= en_dt)
indices_2024 = np.where(mask_2024)[0]
unique_days = np.unique(day_ints[indices_2024])

day_to_indices = {}
for idx in indices_2024:
    d = day_ints[idx]
...
def run_2024_full_backtest():
    ...
    equity_curve = [equity]
    equity_timestamps = [ts_arr[indices_2024[0]]]
    trades = []
...
print("Executing Locked Production Backtest for Full Year 2024...")
tdf, eq_curve, eq_ts = run_2024_full_backtest()
...
# Monthly metrics for all 12 months of 2024
months_2024 = [f"2024-{m:02d}" for m in range(1, 13)]
month_names_2024 = ['Jan', 'Feb', 'Mar', 'Apr', 'Mei', 'Jun', 'Jul', 'Agu', 'Sep', 'Okt', 'Nov', 'Des']
...
# Save results JSON
out_json_path = reports_dir / "backtest_500_modal_2024_full.json"
results_dict = {
    'target_year': 2024,
...
# ================= PLOTTING HIGH-END TEARSHEET FOR 2024 =================
...
ax_eq.plot(eq_dates, eq_curve, color=c_teal, linewidth=2.2, label=f'2024 Locked Production Equity (Final: ${eq_curve[-1]:,.2f})')
...
ax_eq.set_title("EQUITY GROWTH CURVE: $500 CAPITAL (FIXED 0.01 LOT) — FULL YEAR 2024", fontsize=13, fontweight='heavy', color='#ffffff', pad=12)
...
card_text = (
    f"2024 FULL YEAR AUDIT SUMMARY\n"
    f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
...
bars = ax_pnl.bar(month_names_2024, pnl_vals, color=bar_colors, width=0.55, edgecolor='#ffffff', linewidth=0.5, alpha=0.85)
...
ax_pnl.set_title("2024 MONTHLY NET PNL ($ USD) — 12 MONTHS PERFORMANCE", fontsize=11, fontweight='bold', color='#ffffff', pad=10)
...
ax_wr.plot(month_names_2024, wr_vals, marker='o', markersize=5, color=c_gold, linewidth=1.8, label='Win Rate (%)')
ax_wr.axhline(50.0, color=c_red, linestyle='--', linewidth=1.2, label='50% Threshold')

for i, txt in enumerate(wr_vals):
    ax_wr.annotate(f"{txt:.0f}%", (month_names_2024[i], wr_vals[i] + 1.2), ha='center', fontsize=7.5, color='#ffffff', fontweight='bold')

ax_wr.set_title("2024 MONTHLY WIN RATE (%)", fontsize=11, fontweight='bold', color='#ffffff', pad=10)
...
out_png_path = reports_img_dir / "backtest_500_modal_2024_full.png"
plt.savefig(out_png_path, dpi=200, bbox_inches='tight', facecolor=fig.get_facecolor())
plt.close()

# Also save to brain artifact dir
brain_png_path = brain_artifact_dir / "backtest_500_modal_2024_full.png"
import shutil
shutil.copyfile(out_png_path, brain_png_path)

print(f"\nSaved 2024 chart tearsheet to: {out_png_path}")
