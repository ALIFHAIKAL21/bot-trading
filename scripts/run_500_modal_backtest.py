"""
Professional Backtest Simulation: $500 Capital Fixed 0.01 Lot (Jan - Aug 2026)
Pair: XAU/USD M30
Model: MOMENT 15-Channel LoRA Foundation Model
Execution: Single Position 0.01 Lot, Instant BE @ +1.0R, Trailing Stop @ +1.5R, Max TP @ +2.5R
"""
...
# Strict Parameters for $500 Account
...
CONTRACT_SIZE = 100.0          # 1 lot = 100 oz (0.01 lot = 1 oz)
...
def calc_friction(lot=FIXED_LOT):
    # Spread + 2-way slippage + broker commission
    return round(SPREAD_PRICE * lot * CONTRACT_SIZE + SLIPPAGE_PRICE * lot * CONTRACT_SIZE * 2 + COMMISSION_PER_LOT * lot, 3)
...
def run_500_backtest(tau_val=0.32):
    equity = INITIAL_EQUITY
    equity_curve = [equity]
    equity_timestamps = [ts_arr[indices_2026[0]]]
    trades = []
    
    friction_cost = calc_friction(FIXED_LOT) # ~$0.17 per 0.01 lot trade
                ...
                for f_idx in range(best_idx + 1, min(best_idx + 16, len(df))):
                    ...
                    if d == "BUY":
                        ...
                        # Check Max TP
                            ...
                            # Activate Breakeven if target 1.0R reached
                            if not be_activated and f_high >= be_trigger_p:
                                be_activated = True
                                f_buff = SPREAD_PRICE + (COMMISSION_PER_LOT / CONTRACT_SIZE) # +$0.11 buffer
                                cur_sl = max(cur_sl, round(ep + f_buff, 2))
                        ...
                        # Check Max TP
                            ...
                            if not be_activated and f_low <= be_trigger_p:
                                be_activated = True
                                f_buff = SPREAD_PRICE + (COMMISSION_PER_LOT / CONTRACT_SIZE)
                                cur_sl = min(cur_sl, round(ep - f_buff, 2))
                            # Activate Trailing Stop if > 1.5R
...
print("Executing Backtest for Modal $500, Lot 0.01 Fixed (Jan - Aug 2026)...")
tdf, eq_curve, eq_ts = run_500_backtest(tau_val=0.32)
...
print("\n" + "="*70)
print(f"HASIL BACKTEST RESMI: MODAL $500 (LOT 0.01 FLAT) - JANUARI S/D AGUSTUS 2026")
print("="*70)
print(f"Modal Awal            : ${INITIAL_EQUITY:,.2f}")
print(f"Saldo Akhir           : ${eq_curve[-1]:,.2f}")
print(f"Net Profit Bersih     : ${net_pnl:+,.2f} ({return_pct:+.2f}%)")
print(f"Titik Saldo Terendah  : ${min_equity:,.2f} (Akun tetap aman, tidak pernah MC)")
...
print(f"Akurasi (Win Rate)    : {win_rate:.2f}% ({len(wins)} Menang / {len(losses)} Kalah)")
...
# Save JSON Report
audit_data = {
    "engine": "MOMENT-15ch Professional Simulator (Fixed 0.01 Lot Anchor)",
...
json_path = reports_dir / "backtest_500_modal_001_lot_2026.json"
...
# ==============================================================================
print("\nRendering Masterpiece Institutional Visual Report PNG for $500 Account...")
...
# Header Title Block
fig.text(0.04, 0.968, "XAU/USD M30 REALISTIC BACKTEST: MODAL $500 (LOT 0.01 FLAT)",
         fontsize=16, fontweight='bold', color=TEXT_MAIN, ha='left')
fig.text(0.04, 0.942, f"Modal Awal: $500.00 USD  |  Lot Flat: 0.01 (Tanpa Kenaikan Lot)  |  Frekuensi Sehat: {trades_per_day:.2f} Entri/Hari  |  Periode: Jan - Agu 2026",
...
kpis = [
    ("MODAL AWAL", "$500.00", "Ukuran Akun Retail Asli", TEXT_MAIN),
    ("SALDO AKHIR", f"${eq_curve[-1]:,.2f}", f"+${net_pnl:,.2f} (+{return_pct:.1f}%)", GREEN_COL),
    ("LOT FLAT", "0.01 Lot", "Tanpa Kenaikan Lot (Fixed)", BLUE_COL),
...
ax_m.set_title("PERFORMA BULANAN (MONTHLY NET PnL) - MODAL $500", fontsize=11, fontweight='bold', color=TEXT_MAIN, pad=10, loc='left')
...
# Save PNG
out_png_proj = reports_img_dir / "backtest_500_modal_001_lot_report.png"
out_png_brain = brain_artifact_dir / "backtest_500_modal_001_lot_report.png"
