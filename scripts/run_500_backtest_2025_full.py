"""
Production Institutional Backtest: FULL YEAR 2025 (All 12 Months)
...
print("Executing Locked Production Backtest for Full Year 2025...")
tdf, eq_curve, eq_ts = run_2025_full_backtest()
...
# Monthly metrics for all 12 months of 2025
months_2025 = [f"2025-{m:02d}" for m in range(1, 13)]
month_names_2025 = ['Jan', 'Feb', 'Mar', 'Apr', 'Mei', 'Jun', 'Jul', 'Agu', 'Sep', 'Okt', 'Nov', 'Des']

monthly_summary = []
running_bal = INITIAL_EQUITY
for m_str in months_2025:
...
# Save results JSON
out_json_path = reports_dir / "backtest_500_modal_2025_full.json"
results_dict = {
    'target_year': 2025,
...
# ================= PLOTTING HIGH-END TEARSHEET FOR 2025 =================
...
ax_eq.plot(eq_dates, eq_curve, color=c_teal, linewidth=2.2, label=f'2025 Locked Production Equity (Final: ${eq_curve[-1]:,.2f})')
...
ax_eq.set_title("EQUITY GROWTH CURVE: $500 CAPITAL (FIXED 0.01 LOT) — FULL YEAR 2025", fontsize=13, fontweight='heavy', color='#ffffff', pad=12)
...
card_text = (
    f"2025 FULL YEAR AUDIT SUMMARY\n"
    f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
...
bars = ax_pnl.bar(month_names_2025, pnl_vals, color=bar_colors, width=0.55, edgecolor='#ffffff', linewidth=0.5, alpha=0.85)
...
ax_pnl.set_title("2025 MONTHLY NET PNL ($ USD) — 12 MONTHS PERFORMANCE", fontsize=11, fontweight='bold', color='#ffffff', pad=10)
...
ax_wr.set_title("2025 MONTHLY WIN RATE (%)", fontsize=11, fontweight='bold', color='#ffffff', pad=10)
...
out_png_path = reports_img_dir / "backtest_500_modal_2025_full.png"
plt.savefig(out_png_path, dpi=200, bbox_inches='tight', facecolor=fig.get_facecolor())
plt.close()

# Also save to brain artifact dir
brain_png_path = brain_artifact_dir / "backtest_500_modal_2025_full.png"
import shutil
shutil.copyfile(out_png_path, brain_png_path)

print(f"\nSaved 2025 chart tearsheet to: {out_png_path}")
