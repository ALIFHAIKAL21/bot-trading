"""
Test different trade cadence mechanisms for 4-5 entries/day on 2026 Jan-Aug with Model Baru (15ch)
"""
import numpy as np
import pandas as pd
import pathlib
...
# Let's test simulation with macro/order block filter and conf > p_hold
def run_simulation(tau_val=0.355, max_daily=5, max_concurrency=2, use_filter=True, sizing="fixed_002"):
    ...
    for bar_idx in range(len(df)):
        ...
        if pred_idx >= 0 and pred_idx < len(preds_15ch) and len(open_positions) < max_concurrency and daily_count < max_daily:
            ...
            if not fri_freeze and not lon_quar:
                ...
                if conf >= tau_val and (not use_filter or conf > p_hold):
...
print("Testing with macro filter and conf > p_hold...")
for tau in [0.30, 0.32, 0.34, 0.35, 0.36, 0.38]:
    for max_d in [4, 5]:
        for conc in [1, 2]:
            for sz in ["fixed_002", "dynamic_1.5"]:
                res = run_simulation(tau_val=tau, max_daily=max_d, max_concurrency=conc, use_filter=True, sizing=sz)
                if res and res['trades'] > 100:
                    print(f"Tau {tau:.2f} | Dly {max_d} | Cnc {conc} | Sz {sz:>11} | Tr {res['trades']:4d} ({res['trades_per_day']:.2f}/d, {res['trades_per_active_day']:.2f}/act) | WR {res['win_rate']:5.1f}% | PF {res['pf']:5.2f} | PnL ${res['net_pnl']:8.2f} ({res['return_pct']:6.1f}%) | DD {res['max_dd']:5.2f}%")
