...
print(f"2024 Total Trades: {len(tdf)}")
buys = tdf[tdf['direction'] == 'BUY']
sells = tdf[tdf['direction'] == 'SELL']

print(f"BUY Trades: {len(buys)}, Win Rate: {(buys['pnl_net'] > 0).mean()*100:.1f}%, PnL: ${buys['pnl_net'].sum():+,.2f}")
print(f"SELL Trades: {len(sells)}, Win Rate: {(sells['pnl_net'] > 0).mean()*100:.1f}%, PnL: ${sells['pnl_net'].sum():+,.2f}")
